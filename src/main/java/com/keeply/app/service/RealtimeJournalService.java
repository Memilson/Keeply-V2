...
/**
 * RealtimeJournalService:
 * - Linux: WatchService (normalmente inotify por baixo) -> eventos em tempo real
 * - Windows: WatchService agora; depois pluga USN (NTFS) para "journal de verdade"
 *
 * VSS não detecta mudança: ele entra na fase de "ler arquivo para backup" com consistência.
 */
public final class RealtimeJournalService implements Closeable {
    ...
    public RealtimeJournalService(Path root, Consumer<FsChange> sink) {
    ...
    public void start() throws IOException {
        ...
        // Hoje: WatchService funciona “já”.
        // Amanhã: se Windows + NTFS + flag habilitada, troca para USNSource.
        ...
        void loop() {
            while (alive.get()) {
                ...
                for (WatchEvent<?> ev : key.pollEvents()) {
                    ...
                    if (k == OVERFLOW) {
                        // overflow = fila estourou => pode ter perdido eventos.
                        // Em modo "sem scan", a saída correta é:
                        // - se tiver USN: fazer catch-up pelo journal
                        // - se não tiver: reconciliar pelo menos o diretório afetado
                    ...
                    // Se criou pasta, registra recursivo (senão perde mudanças dentro dela)
    ...
    //
    // Fonte: leitura via FSCTL_QUERY_USN_JOURNAL + FSCTL_READ_USN_JOURNAL.
    // Você persiste (lastUsn + journalId) e faz catch-up após restart/overflow.
    ...
    // VSS é COM/API; entra na fase de "ler arquivo para backup/hash".
